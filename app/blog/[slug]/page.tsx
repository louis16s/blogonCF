...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="blog-frame article-frame"><SiteSidebar /><main className="article-shell"><article><ArticleClient slug={decoded} initialPost={payload?.post} initialBlocks={payload?.locked ? [] : payload?.blocks || []} initialLocked={Boolean(payload?.locked)} initialFetched={fetched} initialError={payload?.error || ""} initialTruncated={Boolean(payload?.truncated)} /></article><nav className="article-return" aria-label="文章结束导航"><Link href="/">← 返回全部文章</Link></nav><ContentFooter /></main></div>;
}
