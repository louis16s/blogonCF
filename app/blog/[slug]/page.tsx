...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="blog-frame article-frame"><SiteSidebar /><main className="article-shell"><article><ArticleClient slug={decoded} initialPost={payload?.post} initialBlocks={payload?.locked ? [] : payload?.blocks || []} initialNextCursor={payload?.locked ? undefined : payload?.nextCursor} initialLocked={Boolean(payload?.locked)} initialFetched={fetched} initialError={payload?.error || ""} initialTruncated={Boolean(payload?.truncated)} /></article><ContentFooter /></main></div>;
}
